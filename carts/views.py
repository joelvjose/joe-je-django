...
def cart(request, total=0, quantity= 0, cart_items=None):
    ...
    try:
        if request.user.is_authenticated:
            ...
            cart_items = CartItems.objects.filter(cart= cart.id,is_active = True)
    ...
    return render(request,"joejee/shopping-cart.html",context)
...
def add_cart(request, product_id):
    ...
    if current_user.is_authenticated:
        ...
        is_cart_item_exist = CartItems.objects.filter(product = product,user = current_user).exists()
        ...
        if is_cart_item_exist:
            ...
            # existing variation from ddb
            # curent variation from browser
            # item id from db
            ex_var_list=[]
            id=[]
            for item in cart_item:
